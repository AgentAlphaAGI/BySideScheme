...
*   **🏢 全局势建模**: 数字化你的职场环境，支持**多维度角色 (Stakeholders)** 分析。不只是针对老板，还要看隔壁组长、大老板等复杂关系，综合博弈。
*   **🗣️ 策略与话术生成**: 针对同一事件，生成“对上汇报”、“自我复盘”和“下一步策略”三层输出。
*   **🎭 职场模拟器**: 基于 AutoGen 的多 Agent 角色扮演。在真实汇报前，先在模拟器里和 AI 扮演的“暴躁老板”或“甩锅同事”过两招。
*   **🧠 智能长期记忆**: 记住你和老板的每一次互动，自动提炼长期洞察（Insights），越用越懂你。
*   **🔒 隐私优先**: 核心数据（局势、记忆向量）完全本地化存储，拒绝云端泄露风险。
...
访问 `http://localhost:5173` (或终端提示的其他端口) 即可开始体验。
