...
class AdvisorService:
    ...
    def process_daily_input(self, user_id: str, fact: str, situation: SituationModel) -> Dict[str, Any]:
        ...
        decision = self.decision_engine.evaluate(fact, situation_context, memory_context)
        
        # 3. 生成阶段
        logger.debug(f"[{user_id}] Generating narratives...")
        narrative = self.narrative_generator.generate(fact, decision, situation_context, memory_context)
